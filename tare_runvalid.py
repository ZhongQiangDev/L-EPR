...
def evalacc(model, dev_set):
    ...
    for devBatch in tqdm(devloader):
        ...
        with torch.no_grad():
            l, pre = model(devBatch[0], devBatch[1], devBatch[2], devBatch[3], devBatch[4], devBatch[6], devBatch[7], devBatch[8], devBatch[9], devBatch[10], devBatch[11], tmpf,
                           tmpc, tmpindex, tmpchar, tmpindex2, rulead, antimask2, devBatch[5])
            ploss.append(l)
            print(l)
...
def pretrain():
    ...
    for epoch in range(100000):
        ...
        for dBatch in tqdm(data_loader):
            ...
            loss = torch.mean(
                loss)  # + F.max_pool1d(loss.unsqueeze(0).unsqueeze(0), 2, 1).squeeze(0).squeeze(0).mean() + F.max_pool1d(loss.unsqueeze(0).unsqueeze(0), 3, 1).squeeze(0).squeeze(0).mean() + F.max_pool1d(loss.unsqueeze(0).unsqueeze(0), 4, 1).squeeze(0).squeeze(0).mean()
            # optimizer.zero_grad()
            if j % 100 == 0:
                closs = loss.item()
...
def train():
    ...
    for epoch in range(100000):
        ...
        for dBatch in tqdm(data_loader):
            ...
            for i in range(len(dBatch)):
                dBatch[i] = gVar(dBatch[i])
            loss, _ = model(dBatch[0], dBatch[1], dBatch[2], dBatch[3], dBatch[4], dBatch[6], dBatch[7], dBatch[8], dBatch[9], dBatch[10], dBatch[11], tmpf, tmpc, tmpindex,
                            tmpchar, tmpindex2, rulead, antimask2, dBatch[5])
            loss = torch.mean(
                loss)  # + F.max_pool1d(loss.unsqueeze(0).unsqueeze(0), 2, 1).squeeze(0).squeeze(0).mean() + F.max_pool1d(loss.unsqueeze(0).unsqueeze(0), 3, 1).squeeze(0).squeeze(0).mean() + F.max_pool1d(loss.unsqueeze(0).unsqueeze(0), 4, 1).squeeze(0).squeeze(0).mean()
            # optimizer.zero_grad()
            # print(loss)
    ...
    with torch.no_grad():
        ...
        for lenidx in tqdm(range(args.CodeLen), desc="BeanSearch-batch%d" % k):
            ...
            for p in range(beamsize):
                ...
                result = model.pre_forward(gVar(inputnl[0][validnum]).view(-1, args.NlLen), gVar(inputnl[1][validnum]).view(-1, args.NlLen + args.varlen, args.NlLen + args.varlen),
                                           gVar(tmprule), gVar(tmpruleparent), gVar(tmprulechild), gVar(tmpAd), gVar(tmptreepath), gVar(inputnl[8][validnum]).view(-1, args.NlLen),
                                           gVar(inputnl[9][validnum]).view(-1, args.varlen), gVar(inputnl[10][validnum]).view(-1, args.varlen),
                                           gVar(inputnl[11][validnum]).view(-1, args.varlen), tmpf, tmpc, tmpindex, tmpchar, tmpindex2, rulead, antimasks, nlencode[0][validnum],
                                           nlencode[1][validnum], None, "test")
                ...
                for j in range(batch_size):
                    ...
                    if 'root StatementExpression expression MethodInvocation qualifier otherwise_ter ^ ^ arguments BinaryOperation operator -_ter ^ ^ operandl MemberReference qualifier digit_list_ter ^ ^ member length_ter ^ ^ ^ ^ operandr Literal value 1_ter ^ ^ ^ ^ ^ Literal value' in x.root.printTree(
                            x.root):
                        a = 1
                    if index == 7:
                        a = 1
    ...
    if len(nodes) >= 2:
        ...
        if unknown.father.father and unknown.father.father.namewithouttype == 'MethodInvocation':
            ...
            if unknown.father.father.father.father and (
                    unknown.father.father.father.father.namewithouttype == 'MethodInvocation' or unknown.father.father.father.father.namewithouttype == 'ClassCreator') and ans.type == "":
                mname = ""
                tname = ""
    ...
    for x in tqdm(devloader):
        ...
        for i in tqdm(range(len(ans))):
            ...
            if os.path.exists("Valid/names/%s.json" % idss):
                classcontent = [json.load(open("Valid/names/%s.json" % idss, 'r'))]
            else:
                assert 0
            ...
            for j in (range(len(ans[i]))):
                ...
                for code in tcodes:
                    ...
                    if str(mode) + code + str(data[currid]['line']) not in patch:
                        ...
                        continue
                    savedata.append({'id': currid, 'idss': idss, 'precode': data[currid]['precode'], 'aftercode': data[currid]['aftercode'], 'oldcode': data[currid]['oldcode'],
                                     'filename': data[currid]['filepath'], 'mode': mode, 'code': code, 'line': data[currid]['line'], 'isa': data[currid]['isa'],
                                     'index': len(savedata), 'prob': ans[i][j].prob})
        indexs += 1
