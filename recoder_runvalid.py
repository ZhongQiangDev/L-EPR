            ...
            for ba in range(batch_size):
                ...
                for j in range(beamsize):
                    ...
                    for i in range(len(indexs)):
                        ...
                        prob = x.prob + np.log(
                            cresult[indexs[i]])  # copynode.prob = copynode.prob + np.log(cresult[indexs[i]])
            ...
            for j in range(len(ans[i])):
                ...
                for code in tcodes:
                    ...
                    if str(mode) + code + str(data[currid]['line']) not in patch:
                        ...
                        continue
                    savedata.append({'id': currid, 'idss': idss, 'precode': data[currid]['precode'],
                                     'aftercode': data[currid]['aftercode'], 'oldcode': data[currid]['oldcode'],
...
def solveone2(data, model):  # (treestr, prob, model, subroot, vardic, typedic, idx, idss, classname, mode):
    ...
    for x in tqdm(devloader):
        ...
        ans = BeamSearch((x[0], x[1], None, None, None, None, None, None, x[2], x[3]), dev_set, model, 60,
                         args.batch_size, indexs)
        ...
        for i in range(len(ans)):
            ...
            for j in range(len(ans[i])):
                ...
                if not an:
                    print('debug2')
                    continue
                savedata.append({'precode': data[currid]['precode'], 'aftercode': data[currid]['aftercode'],
                                 'oldcode': data[currid]['oldcode'], 'mode': mode,
                                 'code': ans[i][j].root.printTree(ans[i][j].solveroot), 'line': data[currid]['line']})
    # print(savedata)
    return savedata
    # for x in savedata:
